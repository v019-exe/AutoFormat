...
download_distro() {
  ...
  if [[ -n "${!distro_versions[${version_key}]}" ]]; then
    echo -e "[VOID][$(date +"%H:%M:%S")]: Descargando $distro_name $version"
    wget -O "$distro_name-$version.iso" "${!distro_versions[${version_key}]}"
    if [ $? -eq 0 ]; then
      echo -e "[VOID][$(date +"%H:%M:%S")]: Descargado con éxito"
    else
      echo -e "[VOID][$(date +"%H:%M:%S")]: Fallo en la descarga de $distro_name $version, verifica tu conexión"
      exit 1
    fi
  else
    echo -e "[VOID][$(date +"%H:%M:%S")]: La versión $version no está disponible para $distro_name."
    exit 1
  fi
}
